...
export default function InvestmentsClient({ slug }: { slug: string }) {
        ...
        await sendWeb3Form(
          {
            name: callbackName,
            phone: callbackPhone,
            investment_product: data.title,
            monthly_investment: `₹${monthly.toLocaleString("en-IN")}`,
            investment_tenure: `${years} Years`,
            expected_returns: `₹${wealthRes.returns}`,
            estimated_wealth: `₹${wealthRes.total}`,
          },
          `New Investment Advisory Request: ${data.title} - Finsocap`
        );
      } catch (err) {
        console.error(err);
      }
